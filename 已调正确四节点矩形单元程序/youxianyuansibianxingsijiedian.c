...
#define  NZ 8
#define NPF 12
#define nl 12    //板长方向划分个数
#define nb 6     //板高方向划分个数
#define  NE  (nl*nb)
#define  NJ  ((nl+1)*(nb+1))
...
double AJZ[NJ+1][3];
int JM[NE+1][5];                    //={{0,0,0,0,0},{0,2,3,4,1},{0,3,5,6,4}};
...
int NZC[NZ+1]={0,1,3,5,7,9,11,13,170};
...
{	
					...
					if(mm==1)
				{
					...
					//BBYY=-1.0;
				...
				
						{
								...
								fprintf(fp,"\n%f    %f   %f    %f    %f    %f    %f    %f",S[i][1],S[i][2],S[i][3],S[i][4],S[i][5],S[i][6],S[i][7],S[i][8]);
						}
					fprintf(fp,"\n单元左下角应力i节点\n");
					jdh=JM[E][1];
	...
	for(i=1;i<=NJ;i++)
	{
		...
		fprintf(fp,"绕点平均法    节点%-3d      sx应力为%-18.4f     sy应力为%-18.4f       tou应力为%-18.4f\n",i,SSS[1][i],SSS[2][i],SSS[3][i]);
	}
